...
app.get('/cases/:customer_id', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `SELECT case_name, case_status, expected_revenue, representative
       FROM cases
       WHERE customer_id = $1
...
// 案件IDで案件詳細を取得
app.get('/case/:customer_id', async (req, res) => {
  const customer_id = req.params.customer_id;
  try {
    const result = await pool.query(
    ...
    console.error('案件詳細取得エラー:', err);
    res.status(500).json({ error: '案件詳細の取得に失敗しました' });
  }
});
